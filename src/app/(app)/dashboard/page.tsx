...
export default function DashboardPage() {
  ...
  const ActionHubCard = () => (
    <Card className="rounded-2xl border-0 flex flex-col">
      <CardHeader>
      ...
      <CardContent className="flex-1 flex flex-col">
        <Tabs defaultValue="proximos" className="w-full flex flex-col flex-1">
          ...
          <TabsContent value="hoje" className="mt-4 flex-1">
            {isLoadingWeeklyPlans ? (
              ...
              <div className="text-center py-8 flex flex-col items-center justify-center h-full"><p className="text-muted-foreground text-sm">Nenhuma tarefa no plano para hoje.</p><Button variant="link" asChild><Link href="/generate-weekly-plan">Gerar Novo Plano</Link></Button></div>
            )}
          </TabsContent>
           <TabsContent value="posts" className="mt-4 flex-1">
             {isFetchingPosts ? (
                <div className="flex justify-center items-center h-full"><Loader2 className="h-8 w-8 animate-spin text-primary" /></div>
             ) : (
