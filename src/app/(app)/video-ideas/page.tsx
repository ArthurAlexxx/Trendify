    ...
    <div className="space-y-8">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab} className="w-full">
        <TabsList className="grid w-full grid-cols-3 bg-muted p-1">
          <TabsTrigger value="generate" className="text-muted-foreground data-[state=active]:bg-background data-[state=active]:text-primary px-6 py-2 text-sm font-semibold transition-colors hover:bg-primary/5">Gerar Nova Ideia</TabsTrigger>
          <TabsTrigger value="result" disabled={!result && !isGenerating} className="text-muted-foreground data-[state=active]:bg-background data-[state=active]:text-primary px-6 py-2 text-sm font-semibold transition-colors hover:bg-primary/5">
            Resultado
            {isGenerating && <Loader2 className="ml-2 h-4 w-4 animate-spin" />}
          </TabsTrigger>
           <TabsTrigger value="saved" disabled={!videoIdeas || videoIdeas.length === 0} className="text-muted-foreground data-[state=active]:bg-background data-[state=active]:text-primary px-6 py-2 text-sm font-semibold transition-colors hover:bg-primary/5">
            Salvos
                         ...
                         {totalSavedPages > 1 && (
                            <div className="flex justify-center items-center gap-2 pt-4">
                                ...
                                <Button
                                    variant="outline"
                                    size="sm"
                                    onClick={() => setSavedIdeasPage(p => Math.min(totalSavedPages, p + 1))}
